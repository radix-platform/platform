...
#define CN_PC32b(m) ((m) - (((m) >> 1) & 0x55555555))
#define CN_PC32a(m) ((CN_PC32b (m) & 0x33333333) + ((CN_PC32b (m) >> 2) & 0x33333333))
#define CN_PC32(m) ((((uint64_t)((CN_PC32a (m) & 0x0F0F0F0F)		\
			      + ((CN_PC32a (m) >> 4) & 0x0F0F0F0F))	\
		   * 0x01010101) >> 24) & 0xFF)
...
#define PF_RGB(tn, vn, pf, pfxbo, pfxrb, pfxa, vpf, bo, b, g, r, a)	\
	[pf] = {							\
		...
		.v4l2_fourcc = vpf,					\
		.byte_order = bo,					\
		.bits_per_pixel = CN_PC32 ((b) | (g) | (r) | (a)),	\
		.color_depth = CN_PC32 ((b) | (g) | (r)),		\
		.mask = { b, g, r, a },					\
		.n_bits = { CN_PC32 (b), CN_PC32 (g), CN_PC32 (r), CN_PC32 (a) },	\
