...
#
# Do not try to execute cross assembled object file
#
#       my $absexefile = File::Spec->rel2abs($exefile);
#       $absexefile = '"'.$absexefile.'"' if $absexefile =~ m/\s/;
#       push @wrongresult, $lib if $rv == 0 && -x $exefile && system($absexefile) != 0;
        push @wrongresult, $lib if ! -x $exefile;
...
sub _findcc {
    ...
    my $cc      = "$ENV{CC}";
    my $ld      = "$ENV{LD}";
    my $ccflags = "$ENV{CFLAGS}";
    my $ldflags = "$ENV{LDFLAGS}";

    # Need to use $keep=1 to work with MSWin32 backslashes and quotes
    my $Config_ccflags =  $ccflags;  # use copy so ASPerl will compile
    my @Config_ldflags = ();
    for my $config_val ( $ldflags ){
