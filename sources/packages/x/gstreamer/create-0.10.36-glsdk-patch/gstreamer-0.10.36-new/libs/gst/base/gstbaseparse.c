...
gst_base_parse_chain (GstPad * pad, GstBuffer * buffer)
{
  ...
  if (G_LIKELY (buffer)) {
    GST_LOG_OBJECT (parse, "buffer size: %u, offset = %" G_GINT64_FORMAT,
  ...
  while (!parse->priv->flushing) {
    ...
    for (;;) {
      ...
      if (G_UNLIKELY (parse->priv->drain)) {
        min_size = av;
        GST_DEBUG_OBJECT (parse, "draining, data left: %u", min_size);
      ...
      if (av < min_size) {
        GST_DEBUG_OBJECT (parse, "not enough data available (only %u bytes)",
            av);
        gst_buffer_unref (tmpbuf);
        goto done;
      }
      ...
      if (res) {
        if (gst_adapter_available (parse->priv->adapter) < fsize) {
          GST_DEBUG_OBJECT (parse,
              "found valid frame but not enough data available (only %u bytes)",
              gst_adapter_available (parse->priv->adapter));
          gst_buffer_unref (tmpbuf);
          goto done;
        }
        GST_LOG_OBJECT (parse, "valid frame of size %u at pos %d", fsize, skip);
        break;
      }
      if (skip == -1) {
      ...
      if (skip > 0) {
        ...
        old_min_size = 0;
      } else if (parse->priv->drain) {
        /* If we are draining, but the parse wants more data for the same
           start position, then we will not be able to supply more now,
           we're probably looking at a truncated stream. */
        GST_DEBUG_OBJECT (parse, "Draining, and not enough data to parse next");
        gst_buffer_unref (tmpbuf);
        goto done;
      }
...
invalid_min:
  {
    GST_ELEMENT_ERROR (parse, STREAM, FAILED, (NULL),
        ("min_size evolution %u -> %u; breaking to avoid looping",
            old_min_size, min_size));
    return GST_FLOW_ERROR;
  }
}
