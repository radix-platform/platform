...
#ifdef __CYGWIN__
  char posix_filename[PATH_MAX];
#endif

  on_disk_filename = g_filename_from_utf8 (filename, -1, NULL, NULL, NULL);
#ifdef __CYGWIN__
  cygwin_conv_to_full_posix_path (on_disk_filename, posix_filename);
  status = stat (posix_filename, &filestat);
#else
...
#ifdef __CYGWIN__
  cygwin_conv_to_full_posix_path (on_disk_directory, posix_directory);
  if (stat (posix_directory, &filestat) != 0)
#else
...
#else
#ifdef __CYGWIN__
	      if (mkdir (posix_directory, 0777) != 0)
#else
					    ...
					    gint   *project_num_return)
{
  ...
  max_project_num = 0;
#ifdef __CYGWIN__
  cygwin_conv_to_full_posix_path (projects_dir, posix_dir);
  directory = opendir (posix_dir);
#else
