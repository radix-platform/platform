	...
	if (osd_hw.color_info[OSD1] != NULL) {
		...
		VSYNCOSD_WR_MPEG_REG(VIU_OSD1_BLK3_CFG_W0, data32);
		{
			...
			else
			{
				 ...
				 */
				VSYNCOSD_CLR_MPEG_REG_MASK(VPP_MISC, 1<<9); /* disable OSD1 premultiplied alpha */
			}
		}
	}
	...
	if (osd_hw.color_info[OSD2] != NULL) {
		...
		VSYNCOSD_WR_MPEG_REG(VIU_OSD2_BLK0_CFG_W0, data32);
		{
			...
			if (idx >= COLOR_INDEX_32_BGRX && idx <= COLOR_INDEX_32_XRGB)
			{
				 ...
				 */
				VSYNCOSD_SET_MPEG_REG_MASK(VPP_MISC, 1<<8); /* enable OSD2 premultiplied alpha */
			}
			else
			{
				 ...
				 */
				VSYNCOSD_CLR_MPEG_REG_MASK(VPP_MISC, 1<<8); /* disable OSD2 premultiplied alpha */
			}
		}
	}
