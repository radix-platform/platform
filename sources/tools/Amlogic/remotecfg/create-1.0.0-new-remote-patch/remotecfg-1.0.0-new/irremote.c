...
unsigned short key_map[MAX_KEY_CODES], repeat_key_map[MAX_KEY_CODES], mouse_map[MAX_MOUSE_CODES];
unsigned int   factory_customercode_map[MAX_FACT_CUST_CODES];

unsigned short default_key_map[MAX_KEY_CODES] = {
...
unsigned short default_mouse_map[MAX_MOUSE_CODES] = {
 //0x10, 0x11, 0x0b, 0x0e 
 0xffff, 0xffff, 0xffff, 0xffff
};

unsigned short adc_map[MAX_ADC_CODES] = {0xffff, 0xffff}; // left, right
...
int main(int argc, char* argv[])
{
    ...
    for(i = 0; i < MAX_MOUSE_CODES; i++)
        mouse_map[i] = 0xffff;
    for(i = 0; i < MAX_FACT_CUST_CODES; i++)
        factory_customercode_map[i] = 0xffffffff;
    ...
    if(argc < 2){
        ...
        memset(factory_customercode_map, 0xffffffff, MAX_FACT_CUST_CODES);
        }
    else if(argv[1][0]=='-'){
    ...
    for(cci = 0; cci < MAX_FACT_CUST_CODES; cci++) {
        // printf("factory_customercode_map[%lu] = 0x%08x\n", cci, factory_customercode_map[cci]);
        if(remote->factory_customercode_map[cci] != 0xffffffff){
            ioctl(device_fd, REMOTE_IOC_INFCODE_CONFIG, &cci);

            val = remote->factory_customercode_map[cci] & 0xffff;
            printf("Customer code [%d] = 0x%04x\n", cci, val);
            remote->factory_code = val;
