...
#undef	prtlong
	...
	*le = '\0';
    ...
    if (help)
	(void)POPT_fprintf(fp,"  %-*s   ", (int)(maxLeftCol+displaypad), left);
    else {
	(void)POPT_fprintf(fp,"  %s\n", left); 
	goto out;
    }
	...
	{   char * fmthelp = xstrdup(help);
		...
		sprintf(format, "%%s\n%%%ds", (int) indentLength);
		/*@-formatconst@*/
		(void)POPT_fprintf(fp, format, fmthelp, " ");
		/*@=formatconst@*/
		free(fmthelp);
	    }
	}
	...
	/*@modifies fp, fileSystem @*/
{
    ...
    (void)POPT_fprintf(fp, POPT_("Usage:"));
