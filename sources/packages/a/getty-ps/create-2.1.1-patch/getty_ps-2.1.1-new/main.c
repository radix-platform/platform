...
/* speedtab table for modem speeds @B */

struct speedtab {
	ulong	cbaud;		/* baud rate */
	int	nspeed;		/* speed in numeric format */
	char	*speed;		/* speed in display format */
} speedtab[] = {
	{ B300,    300,   "300"  }, /* 300 baud TELEX */
	{ B600,    600,   "600"  },
	{ B1200,  1200,  "1200"  },
	{ B2400,  2400,  "2400"  },
	{ B4800,  4800,  "4800"  },
	{ B9600,  9600,  "9600"  }, /* standard VT320 */
#ifdef  B19200
        { B19200, 19200, "19200" },
#endif  /* B19200 */
#ifdef  B38400
...
#endif  /* B57600 */
#ifdef  B115200
        { B115200, 115200, "115200" }, /* max using 1.8432 MHz XTAL */
#endif  /* B115200 */
#ifdef  B230400
	...
	if (p = defvalue(def, "VERSION")) {
	  if (*p == '/') {
	    if ((fp = fopen(p, "r"))) {
	      ...
	      if(li>0 && buf[li-1]=='\n') 
	        buf[--li] = '\0';
	      debug(D_RUN, "used %li bytes from %s", li, p);
	      Version = strdup(buf);
	    }
	  } else {
	    Version = strdup(p);
...
/* construct /dev/ names for the lines */
	...
	if(Device[0] == '-') /* dologin() via given stdin, stdout, stderr */ 
	  devname[0]='\0';
	else 
	  snprintf(devname, sizeof(devname), "/dev/%s", Device);

	if(strequal(InitDevice, "unknown")) InitDevice = Device;
	snprintf(initdevname, sizeof(initdevname), "/dev/%s", InitDevice);
...
/* vhangup the line */
	...
	while(((fdhup = open(devname, O_RDWR | O_NDELAY)) < 0)) {
		switch(errno) {
			case EBUSY:
 ...
 */
				...
				break;
			case EAGAIN:
				logerr("open(%s) failed: %s", devname, 
				strerror(errno));
				exit(FAIL);
			default:
				Perror("open");
	...
	ioctl(fdhup, TIOCSCTTY, 1);
	(void) ioctl(fdhup, TCGETS, &termio);
	if(NoHangUp) {
	...
	(void) ioctl(fdhup, TCSETSF, &termio);
	if(! NoHangUp) sleep(2);
	...
	(void) signal(SIGHUP, shangup);
#endif
	timer_set_ms(0);	/* cancel itimer */
	if(close(fdhup)<0) {
	  debug(D_INIT, "Caution %s close(%i): %s", devname, fdhup, 
		strerror(errno));
	}

	/* The init device is opened ONLY if INIT or WAITCHAR is requested. 
	 * The fdhup from above can't be reused for the InitDevice == Device 
	 * case since the read() in waitchar then returns zero bytes EOF 
	 * unless CLOCAL is set.  By reopening the read() blocks, ignores 
	 * DSR and DCD, but SIGHUP is detected (after reading ^@, however).
	 * Note: not closing fdhup until fd is open gives EBUSY when using 
	 * the ttySX<->cuaX logic.  
	 */
	if((init) || (waitchar)) {
	  int i, flags;
	  ...
	  if((fd = open(initdevname, O_RDWR | O_NDELAY)) < 0) {
	    if((errno == EBUSY) || (errno == EAGAIN)) {
	      debug(D_INIT, "line in use... exiting to reinit");
	      exit(0);
	    } else {
	      logerr("open(%s) failed: %s", initdevname, 
		     strerror(errno));
	      exit(FAIL);
	    }
	  }
	  for(i=0; i<3; i++) {     /* stdin stdout sterr */
	    if(fd!=i && dup2(fd,i)<0) {
	      logerr("Error dup2(%i,%i): %s, please report this bug [1]",
		     fd,i, strerror(errno));
	      exit(FAIL);
	    }
	  }
	  if(fd>=i && close(fd)<0) {
	    debug(D_INIT, "Caution %s close(%i): %s", initdevname, fd, 
		  strerror(errno));
	    fd=0;
	  }
...
#endif /* SCHED */
	if(waitchar) {
		/* ibm3153 bootTx: break=^@, 7s delay, RTS DTR ^Q=XON ^F=ACK */
		char *wait = waitfor;
		char ch, buf[3];
		long cnt;
		int chi;
		...
		cnt=read(STDIN, &ch, 1);		/* blocks */
		if(cnt<=0) {                            /* alarm -> EINTR */
		  debug(D_INIT, "Error cnt=%li from read(): %s", 
			cnt, cnt==0 ? "EOF?" : strerror(errno)); 
		  exit(0);
		}

		chi=(unsigned )ch;
		if(chi<' ') {
		  buf[0]='^'; buf[1]=(char )(chi | 0x40); buf[2]='\0';
		} else {
		  buf[0]=ch; buf[1]='\0';
		}
		debug(D_INIT, "read char ch=%i %s", chi, buf);
...
/* now we can lock the line */
	...
	/* read() terminal no data return:  O_NDELAY=0  O_NONBLOCK=-1 EAGAIN */
	if(delay) {
		char ch;
...
/* A terminal generates a 250 ms (or 170 ms) break for one baud change. 
 * This gives a zero character, aka as ^@ or NUL, read from the input stream.
 * The first '\0' yields a speed change -> port reset -> one new '\0'
 * -> new change -> reset -> etc... ==> race condition.  SV.
 * 
 * Remove successive zeros from the input.  The next char is lost if present.
 * while char_count<maximum 
 *   try to read data 
 *   no data ?
 *     sleep for two character delays 
 *     try again to read data 
 *   no data -> quit
 *   nonzero -> quit (No ungetc() since this only works for stdio fread())
 */
int drainbrk(int fdin, long nspeed) {
  int err=0, flags;
  long maxcnt, waitns, cnt;

  maxcnt= nspeed<40 ? 40 : nspeed; /* bit/sec */
  maxcnt/=10; /* char/sec for 10 bit/char == max chars drained for 1 second */
  waitns=2000000000/maxcnt; /* double intercharacter time in nanosecond */

  flags = fcntl(fdin, F_GETFL, 0);
  (void) fcntl(fdin, F_SETFL, flags | O_NDELAY);
  ...
  for(cnt=0; cnt<maxcnt; cnt++) {
    ...
    if(li!=1) {
      ...
      req.tv_sec=0;
      req.tv_nsec=waitns;
      debug(D_RUN, "drainbrk %li nanosleep(%li)", nspeed, req.tv_nsec); 
      if((err=nanosleep(&req, &rem))<0 && errno!=EINTR) {	
	logerr("Error %i nanosleep(%li,%li): %s\n", 
	       err, req.tv_sec, req.tv_nsec, strerror(errno)); 
  ...
  if(cnt>0) {
    long totms=cnt*1000/maxcnt;
    debug(D_RUN, "Drained %li breaks (B%li %li ms)", cnt, nspeed, totms); 
  }

end_error:
  (void) fcntl(fdin, F_SETFL, flags);
  return(err);
}
/*
...
void dologin()
{
	...
	for(;;) {
		...
		for(i=0; speedtab[i].speed[0]!='\0' 
		      && speedtab[i].cbaud!=cbaud; i++);
		Speed = speedtab[i].speed;

		if(drainbrk(STDIN, speedtab[i].nspeed)) 
			exit(FAIL);
		...
		if(fputc('\r', stdout) == EOF) {
		  debug(D_RUN, "Error dologin() stdout EOF"); 
		  exit(0);
		}
...
/*
** timeout: the user types way too damn slow or modem lockup
*/
...
void timeout()
{
	...
	debug(D_RUN, "Caught ALRM signal TimeOut=%d ... exiting", TimeOut);
